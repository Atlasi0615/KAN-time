...
def prepare_like_training(cfg: Dict, split_type: str):
    ...
    return {"features": features, "target": target, "x_scaler": x_scaler, "y_scaler": y_scaler, "train_df": refit_train_df, "test_df": test_df, "X_test_log": X_test_log, "X_test_scaled": X_test_scaled}
...
def summarize_binned(x: np.ndarray, y: np.ndarray, bins: int) -> pd.DataFrame:
    ...
    for i in range(bins):
        ...
        xb = x_sorted[mask]
        yb = y_sorted[mask]
        rows.append({"x_mid_log": float(np.median(xb)), "x_mid": float(np.exp(np.median(xb))), "n": int(mask.sum()), "exp_p10": float(np.quantile(yb, 0.10)), "exp_p50": float(np.quantile(yb, 0.50)), "exp_p90": float(np.quantile(yb, 0.90))})
...
def make_summary_panel(feature: str, out_dir: Path, dpi: int, model_name: str):
    ...
    paths = [out_dir / f"{feature}_response_original.png", out_dir / f"{feature}_response_log.png", out_dir / f"{feature}_local_exponent_reference.png", out_dir / f"{feature}_local_exponent_binned.png"]
...
def gradients_wrt_scaled_inputs(model, x_scaled: np.ndarray, device: str) -> np.ndarray:
    ...
    if isinstance(out, tuple):
        out = out[0]
    y = out[:, 0] if out.ndim == 2 and out.shape[1] == 1 else out.reshape(-1)
    grads = torch.autograd.grad(outputs=y.sum(), inputs=xt, retain_graph=False, create_graph=False)[0]
...
def main():
    ...
    for feat in features:
        ...
        plot_single_feature(feat, x_grid_log, y_grid_log, exp_grid, binned_df, out_dir, args.dpi, "KAN")
        make_summary_panel(feat, out_dir, args.dpi, "KAN")
        rows.append({"feature": feat, "ref_exp_min": float(np.min(exp_grid)), "ref_exp_median": float(np.median(exp_grid)), "ref_exp_max": float(np.max(exp_grid)), "test_exp_p10": float(np.quantile(exponents[:, idx], 0.10)), "test_exp_p50": float(np.quantile(exponents[:, idx], 0.50)), "test_exp_p90": float(np.quantile(exponents[:, idx], 0.90))})
    ...
    print(f"Saved KAN interpretability outputs to: {out_dir}")
